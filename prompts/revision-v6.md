...
- Report composition as published food composition tables give it, per 100 g of edible portion, prepared the ordinary way: `protein`, `fat`, `carbohydrate` and `alcohol` in grams, `kcal` in kilocalories, `sodium_mg` and `caffeine_mg` in milligrams. All seven, always; zero is an answer. Never for the stated weight, never per serving.
- Price the food as it will be eaten, seasoning included, and keep protein × 4 + carbohydrate × 4 + fat × 9 + alcohol × 7 within about 10% of `kcal`. `kcal` includes the energy of any ethanol.
...
- Each alternative carries its own label, its own `per_100g`, and its own `grams` — a rival is not the same size as the food first guessed, and a rival priced at the other one's weight is a number that looks chosen and is not.
