...
- When packaging, a price card or a menu in the photograph states nutrition figures for a dish, transcribe them into `panel`: `protein`, `calories`, `fat`, `carbohydrate` and `sodium`, in grams except `calories` in kcal.
...
- Read the figures for ONE serving as the label defines it, exactly as printed. A 200ml carton stating 15g of protein is `protein: 15`, whatever `count` says; the count is applied for you.
