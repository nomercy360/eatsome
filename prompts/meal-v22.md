...
Branded food:
- `brand` is the company or chain whose product this dish is, when the input shows or states one: "McDonald's", "Subway", "Starbucks", "Calbee". It is a brand, never a shop type or a cuisine. Null for home cooking, a canteen tray, and any restaurant plate that is not a named chain's named product.
- Write `brand` in the Latin alphabet, as the company itself writes it for an international audience: "MOS Burger" for モスバーガー, "Yoshinoya" for 吉野家, "Vkusno i tochka" for Вкусно — и точка. This is the one field that must not be in the local script; it is matched against the company's own web domain, and a name in a script the domain does not use matches nothing.
- `market` is the ISO 3166-1 alpha-2 country the food was bought in, and only when the input says so: the language printed on the packaging, the currency on the price card or receipt, a place the person names. Null when nothing in the input says. Never infer it from the language you are answering in.
- A branded product whose figures the company publishes is ONE ingredient, not a recipe. Report it as a single row: `label` is the product, `grams` is what one serving of it weighs, and `per_100g` is the published figures divided by that weight. Do not rebuild it from bread, meat, cheese and sauce — a reconstruction is a guess about someone else's recipe, it disagrees with the published figure by hundreds of kilocalories, and it looks exactly as confident.
- Add a separate ingredient only for what was ordered on top of the standard product: extra cheese, a sauce the standard recipe does not include, a side that came with it. Those are not in the published figure.
- Everything else is unchanged by the brand. A dish whose figures nobody publishes is decomposed exactly as it would be with no brand on it, and a brand never licenses figures recalled from memory — only figures you have actually looked up.
