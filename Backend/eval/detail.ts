...
const latest = readdirSync(dir)
  ...
  .sort()
  .at(-1) as string;
const rows = readFileSync(join(dir, latest), "utf8")
  .trim()
  .split("\n")
  .map((l) => JSON.parse(l));
...
for (const row of rows) {
  ...
  console.log(
    `${row.caseId}  ${s.pass ? "PASS" : "FAIL"}  recall ${(s.recall * 100).toFixed(0)}%  structure ${(s.structureMatch * 100).toFixed(0)}%  dishes ${s.dishCount}/${s.dishExpected}  counts ${s.countMatch}/${s.countTotal}`,
  );
  ...
  if (s.caffeineExpected != null)
    console.log(`  caffeine: expected ${s.caffeineExpected}mg, derived ${s.caffeineActual}mg`);
  console.log(
    "  golden :",
    golden.dishes.map((d) => `${d.name}${d.count > 1 ? " x" + d.count : ""}`).join(" | "),
  );
  console.log(
    "  answer :",
    answer.dishes
      .map(
        (d: any) =>
          `${d.name}${d.count > 1 ? " x" + d.count : ""}[${d.ingredients.map((i: any) => i.group).join(",")}]`,
      )
      .join(" | "),
  );
}
