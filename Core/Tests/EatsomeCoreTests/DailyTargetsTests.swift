...
struct DailyTargetsTests {
    ...
    @Test("Carbohydrate and fat are one figure each, by a stated rule, and add up to the energy")
    func macroFigures() throws {
        var profile = base
        profile.goal = .loseWeight
        let targets = try #require(DailyTargets.forProfile(profile))
        // Fat is 30% of energy at 9 kcal/g; carbohydrate is what is left.
        #expect(targets.fat == (targets.kcal * 0.30 / 9).rounded())
        #expect(targets.fatRange.contains(targets.fat))
        let accounted = targets.protein * 4 + targets.fat * 9 + targets.carbohydrate * 4
        #expect(abs(accounted - targets.kcal) < 8)
    }
